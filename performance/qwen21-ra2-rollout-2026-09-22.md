...
- Overflow verified 2026-09-22 18:50 NZST: cog `1ba61291-b0b0-4e01-8b98-56b453adb21f` (replaces be939aab/8da66309), `POST /api/cogs/<id>/predict-sync` returned a WebP with `X-AppNZ-Execution-Tier: serverless` in 6.4 s wall / 4.0 s inference (512², 8 steps, warm worker). Qwen gateway `/status.overflow` = {image: true, path /predict-sync, tiers paid}.
- Saturation test (3 concurrent paid 1024² 20-step requests, 1 local permit): req1 local 12.0 s; req2 and req3 relayed to app.nz -> RunPod serverless (`X-AppNZ-Execution-Tier: serverless`), 31.8 s and 37.5 s at the worker (a 4090 streaming weights from the volume, EasyCache 0.05), 32.9 s and 92 s wall because the endpoint had a single worker so req3 queued behind req2. `/status.overflow.saturated` went 0 -> 2. Levers: `workersMax` in the cog/`deploy/runpod-ra2.json` for parallel overflow, worker EasyCache 0.08 (env in `Dockerfile.runpod-sdcpp`), and the pod promotion rule already in app.nz for sustained load.

## Update 19:10 NZST: main gateway routing live

- `omniserve-native.service` (8791) now runs `omniserve-native-ra2overflow/build-full-ra2/omniserve-native` (someone had already switched ExecStart to it) with drop-in `ra2-routing.conf` (`OMNISERVE_NATIVE_IMAGE_MODEL_UPSTREAMS=ra2|qwen-image-2.1|qwen -> 127.0.0.1:8792`, upstream secret, `SD_MAX_VRAM=6`). Restart was health-checked: diffusion/LLM/embeddings ready, `/status.image_model_upstreams` lists the three names, `POST /v1/images/generations {"model":"ra2",...}` on 8791 returns the Qwen image in 10.0 s. Clients can now hit the single 8791 scheduler for both models.
- zimage on 8791 still takes 17-22 s per 1024² because other drop-ins (`zimage-residency.conf` STREAM_LAYERS=1 + unit `PARAMS_BACKEND=cpu`, `overflow-daisy.conf` relaying to daisy's 3090) deliberately keep it streaming; not changed here. Someone also re-ran the Qwen deploy install so 8792 is back on `*=cpu` streaming + EasyCache 0.08 (it measured 10 s via 8791 anyway).
- Other operators are active on prod (netwrck is on `netwrckprod145`, new gateway drop-ins) — re-read state before changing units.
